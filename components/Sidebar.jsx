...
const Sidebar = ({ connectionColor = '#dc2626', setConnectionColor = () => {}, connectionLineType, setConnectionLineType, lastSaved, onSave, onLoad, onClear, onHelp, isDarkMode, toggleTheme, searchTerm, setSearchTerm, onExport, isOpen, onToggle, onOpenCaseManager, currentBoardName, onToggleZen, onNextMatch, onPrevMatch, onToggleBg, onLogout, currentUser, isSyncing, syncError, onRetry, onManualSave }) => {
  ...
  return (
    ...
    >
      <aside
        className={`h-full border-r flex flex-col shadow-2xl backdrop-blur-xl transition-colors duration-500 ease-in-out ${isDarkMode ? 'bg-zinc-900/95 border-zinc-800' : 'bg-white/90 border-gray-200'}`}
        style={{ width: sidebarWidth }}
      >
        ...
        <div className="p-3 space-y-2 overflow-y-auto">
          ...
          <SidebarSection title="Configuración de Hilo" icon={Activity} isDarkMode={isDarkMode} isOpen={sectionsState.connection} onToggle={() => toggleSection('connection')}>
            ...
            <div className="flex gap-1">
              <button onClick={() => setConnectionLineType('straight')} className={`flex-1 py-1.5 rounded border flex justify-center items-center transition-colors ${connectionLineType === 'straight' ? (isDarkMode ? 'bg-zinc-800 border-zinc-600 text-white' : 'bg-white border-gray-300 text-black shadow-sm') : (isDarkMode ? 'border-transparent text-zinc-500 hover:bg-zinc-800/50' : 'border-transparent text-zinc-400 hover:bg-gray-100')}`} title="Línea Recta"><Minus size={14} /></button>
              <button onClick={() => setConnectionLineType('default')} className={`flex-1 py-1.5 rounded border flex justify-center items-center transition-colors ${connectionLineType === 'default' ? (isDarkMode ? 'bg-zinc-800 border-zinc-600 text-white' : 'bg-white border-gray-300 text-black shadow-sm') : (isDarkMode ? 'border-transparent text-zinc-500 hover:bg-zinc-800/50' : 'border-transparent text-zinc-400 hover:bg-gray-100')}`} title="Línea Curva"><Activity size={14} /></button>
              <button onClick={() => setConnectionLineType('step')} className={`flex-1 py-1.5 rounded border flex justify-center items-center transition-colors ${connectionLineType === 'step' ? (isDarkMode ? 'bg-zinc-800 border-zinc-600 text-white' : 'bg-white border-gray-300 text-black shadow-sm') : (isDarkMode ? 'border-transparent text-zinc-500 hover:bg-zinc-800/50' : 'border-transparent text-zinc-400 hover:bg-gray-100')}`} title="Línea Escalonada"><CornerDownRight size={14} /></button>
            </div>
          </SidebarSection>
      ...
      <div
        ...
        className="absolute top-0 right-0 h-full w-2 flex items-center justify-center group"
        style={{ cursor: 'ew-resize', zIndex: 60 }}
        title="Arrastrar para redimensionar"
      >
        {/* Visible bar that appears on hover */}
        <div
          className={`h-full w-1 transition-all duration-150 group-hover:w-1.5 ${isDarkMode ? 'bg-zinc-700 group-hover:bg-red-500' : 'bg-gray-300 group-hover:bg-red-400'}`}
        />
        {/* ↔ icon centered on hover */}
        <div
          className={`absolute opacity-0 group-hover:opacity-100 transition-opacity duration-150 pointer-events-none px-0.5 py-1 rounded text-[10px] font-bold select-none ${isDarkMode ? 'text-red-400' : 'text-red-500'}`}
      ...
      {/* Toggle open/close button — now positioned relative to current width */}
      <button
        onClick={onToggle}
